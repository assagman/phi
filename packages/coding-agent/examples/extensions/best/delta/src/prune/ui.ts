...
export class PruneDashboard {
	...
	private renderDetail(width: number, rows: number): string[] {
		...
		const visibleContent = contentLines.slice(this.detailScrollOffset, this.detailScrollOffset + contentHeight);
	...
	private renderConfirm(width: number, rows: number): string[] {
		...
		lines.push("");
		lines.push(this.centerText(`You are about to delete ${FG_RED}${selected.length}${RESET} items:`, width, ""));
		lines.push("");
		...
		lines.push(this.centerText(`${yesStyle}[ Yes, delete ]${RESET}    ${noStyle}[ No, cancel ]${RESET}`, width, ""));
	...
	private renderCandidateRow(candidate: PruneCandidate, isSelected: boolean, width: number): string {
	...
	private renderDetailFooter(_width: number): string {
		const help = [`${FG_DIM}j/k${RESET} scroll`, `${FG_DIM}space${RESET} toggle`, `${FG_DIM}Esc${RESET} back`].join(
			"  ",
		);
		return ` ${help}`;
	}
