...
import { checkBranchesExist, checkPathsExist, detectBranchRefs, detectFilePaths } from "./detector.js";
import {
...
function analyzeMemory(
	...
	if ((memory.importance === "high" || memory.importance === "critical") && daysSinceUpdate < 60 && !hasLowContent) {
		return null;
	}
	...
	for (const candidate of candidates) {
		...
		if (config.checkFiles && paths.length > 0) {
			...
			if (orphaned.length > 0) {
				enrichedCandidate.reasons = [...new Set([...enrichedCandidate.reasons, "orphaned_path" as PruneReason])];
			}
		}
		...
		if (config.checkBranches && branches.length > 0) {
			...
			if (orphaned.length > 0) {
				enrichedCandidate.reasons = [...new Set([...enrichedCandidate.reasons, "orphaned_branch" as PruneReason])];
			}
		}
