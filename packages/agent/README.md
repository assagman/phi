# @mariozechner/phi-agent

Stateful agent with tool execution and event streaming. Built on `@mariozechner/phi-ai`.

## Installation

```bash
npm install @mariozechner/phi-agent
```

## Quick Start

```typescript
import { Agent } from "@mariozechner/phi-agent";
...
```typescript
declare module "@mariozechner/phi-agent" {
...
```typescript
import { Agent, streamProxy } from "@mariozechner/phi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@mariozechner/phi-agent";
