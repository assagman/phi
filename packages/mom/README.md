...
```bash
npm install @mariozechner/phi-mom
```
...
- run interactive coding agent session: `npx @mariozechner/phi-coding-agent`
- enter `/login` command
...
Update mom anytime with `npm install -g @mariozechner/phi-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
