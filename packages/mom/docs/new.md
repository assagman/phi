...
```typescript
import { type AgentSessionEvent } from "@mariozechner/phi-coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@mariozechner/phi-mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@mariozechner/phi-ai` and `@mariozechner/phi-mom` are aliased to the running mom bundle.
